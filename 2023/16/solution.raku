...
my @photons = [ Photon.new(:x<0>, :y<0>, dir=>E) ];
my $photon;
  ...
  for get-next-photons($photon) -> $next-photon {
    ...
    @photons.push: $next-photon
      unless $next-photon ∈ @photons
        or $next-photon.dir ∈ $record-of-prior-photons; # TODO could also skip if the direction is opposite current direction IFF we still keep the splitter behavior
  }
  # say format-light-path, "\n\n\t{[1 if $_ for @paths].sum}\n\n\n\n\n\n";
}

say format-light-path;
say "\ntotal photons...", $photons-counter, "\n"; # 16275 with twins... 10940 without!
say "\nfield is $w wide • $h high...\n";
say "\n... real-ish-but-flawed count of lit tiles is: {add-em-up}\n";
